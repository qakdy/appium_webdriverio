...
describe('registration test 11 ru; Проверить создание портфолио при выборе на онбординг скрин   радиокнопку "Мастер-пароль и отпечаток пальца/Face ID(рекомендуется)"  или   "Только мастер пароль (наиболее безопасный)"с полями  "Основной пароль" и "Подтвердить пароль"  заполненными разными данными и Измененным полем "Мое портфолио"', function(){
    ...
    it('STEP 1. Изменить данные поля "Мое портфолио"  ', function(){
    ...
    it('STEP 2. Поле  "Основной пароль"  ввести данные', function(){
    ...
    it('STEP 3. В поле  "Подтвердить пароль"  ввести данные отличные от данных введенный в поле "Основной пароль"', function(){
    ...
    it('EXPECTED RESULT. Под полем "Подтвердить пароль" отображается информационное сообщение красным цветом" Пароль не подходит".  ', function(){
