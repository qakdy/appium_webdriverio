...
describe('registration test 14 ru; Проверить создание портфолио при выборе на онбординг скрин   радиокнопку  "Только отпечаток пальца/FaceID(компромиссный вариант)" на экране "Безопастность"? или "Ничего, мне не о чем беспокоится " на экране "Безопастность" с заполненным по умолчанию "Мое портфолио"', function(){
    ...
    it('STEP 1. Оставить поле  "Мое портфолио"  заполненным по умолчанию  ', function(){
    ...
    it('EXPECTED RESULT. После нажатия кнопки [Создать] пользователь успешно зарегистрирован и перешел в основное окно приложения с именем профиля указанным при создании', function(){
